...
public class AppointmentService {
   ...
   public Appointment createAppointment(
      ...
      verifyThatAppointmentTimeIsAvailableForPractitioner(
         createAppointmentRequest);

      var appointment = appointmentsMapper.mapToDatabase(
         createAppointmentRequest);
      appointment = appointmentsRepository.save(appointment);

      var outgoingAppointment = appointmentsMapper.mapToOutgoing(appointment);
      appointmentMessagePublisher.broadcastAppointmentCreated(
         outgoingAppointment);
   ...
   private void verifyThatAppointmentTimeIsAvailableForPractitioner(
      CreateAppointmentRequest createAppointmentRequest
   ) {
   ...
   private List<AppointmentEntity> loadAppointmentsInGivenDate(
      CreateAppointmentRequest createAppointmentRequest
   ) {
      var initialTime = createAppointmentRequest.getStartTime()
         .truncatedTo(ChronoUnit.DAYS);
      var endTime = initialTime.plusDays(1);
      return appointmentsRepository.findByPractitionerIdAndStartTimeBetween(
         createAppointmentRequest.getPractitionerID(),
         initialTime,
         endTime);
   }

   private void checkOverlappingAppointments(
      List<AppointmentEntity> appointments,
      CreateAppointmentRequest createAppointmentRequest
   ) {
      var requestedPeriod = new Period(createAppointmentRequest.getStartTime(),
         createAppointmentRequest.getDuration());
      var periods = appointments.stream().map(appointment -> new Period(
         appointment.getStartTime(),
         appointment.getDuration())).toList();
      if (isPeriodOverlappingOtherPeriods(requestedPeriod, periods)) {
         throw new AppointmentTimeNotAvailableException(
            requestedPeriod.getStart(),
            requestedPeriod.getDuration());
      }
   }

...
@RestController
@RequestMapping(path = "/api/v1/appointments", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
...
public class AppointmentsAPI {
    ...
    @PostMapping
    @ResponseStatus(OK)
    public AppointmentDTO createAppointment(@RequestBody CreateAppointmentRequestDTO createAppointmentRequestDTO) {
        var createAppointmentRequest = appointmentMapper
                .mapToCreateAppointmentRequestFromDTO(createAppointmentRequestDTO);

        var appointment = appointmentService.createAppointment(createAppointmentRequest);
